...
from kpi.config.config import resources_path
from kpi.config.db import db_url
from kpi.config.dev_modus import dev
from kpi.repositories.session import engine, engine_admin
...
def _set_schema(db_dialect: str, connection: Connection) -> None:
    ...
    if db_dialect == "postgresql":
        connection.execute(text("SET search_path TO kpi_schema;"))
    elif db_dialect == "mysql":
        connection.execute(text("USE kpi_schema;"))
...
def _load_csv_mysql(tabelle: str, csv_path: str, connection: Connection) -> None:
    ...
    load_cmd: Final = (
        ...
        if tabelle != "patient"
            ...
            + csv_path
            + "/${tabelle}.csv' INTO TABLE ${tabelle} FIELDS TERMINATED BY ';' ENCLOSED"
            " BY '\"' LINES TERMINATED BY '\\n' IGNORE 1 ROWS (id, version, nachname,"
            " email, kategorie, @has_newsletter, geburtsdatum, homepage, geschlecht,"
            " familienstand, fachaerzte, username, erzeugt, aktualisiert)"
            " SET has_newsletter=cast(@has_newsletter as signed);",
        )
    ).substitute(tabelle=tabelle)
